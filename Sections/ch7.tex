...
\end{equation}
In steady state, angular momentum conservation requires (equation~\eqref{eq:thin_disk_viscous})
\begin{equation}
\nu \Sigma = \frac{\dot{M}}{3\pi}
\left[1 - \left(\frac{R_{\rm in}}{R}\right)^{1/2}\right]
\;\equiv\; \dot{M}\, f^4,
...
\]
The $\alpha$–disk ansatz relates \textbf{viscosity to local pressure and sound speed}:
\[
\nu = \alpha c_s H = \alpha \frac{c_s^2}{\Omega_K}
= \alpha \frac{P}{\rho\,\Omega_K}.
\]
Using vertical hydrostatic equilibrium (\(P \sim \Sigma \Omega_K^2 H\)) and $\rho \sim \Sigma/H$, we can eliminate $\rho$ and $H$:
\begin{align}
\nu &\sim \alpha \frac{P}{(\Sigma/H)\Omega_K}
= \alpha \frac{P H}{\Sigma \Omega_K}, \\
H &\sim \frac{P}{\Sigma \Omega_K^2}
\quad\Rightarrow\quad
\nu \sim \alpha \frac{P^2}{\Sigma^2 \Omega_K^3}.
\end{align}
This important relation shows that, in a radiation-pressure–dominated disk, the \textbf{viscosity depends quadratically on the pressure}.

The radiation pressure gives
\[
P = \frac{a}{3}T_c^4.
\]
From radiative diffusion (equation~\eqref{eq:Tc_Teff_tau}),
\[
...
\[
P \sim \frac{a}{3}
\left(\frac{3}{8}\kappa_{\rm es}\Sigma\right)
\left(\frac{3GM\dot{M}}{8\pi\sigma_{\rm sb}R^3}f^4\right),
\]
or schematically, \(P \propto \Sigma\, \kappa_{\rm es}\, \dot{M}\, \Omega_K^2 f^4.\) Substituting this form of $P$ into $\nu \sim \alpha P^2 \Sigma^{-2}\Omega_K^{-3}$ gives
\[
\nu \sim \alpha\,\kappa_{\rm es}^2\,\dot{M}^2\,\Omega_K\,f^8,
\]
so that, combining with $\nu\Sigma = \dot{M}f^4$,
\begin{equation}
\boxed{
\Sigma \;\sim\;
\frac{1}{\alpha\,\kappa_{\rm es}^2\,\dot{M}\,\Omega_K}\;f^{-4}.
}
\label{eq:sigma_regionA}
\end{equation}
This scaling shows that the surface density \textbf{decreases with increasing accretion rate and $\alpha$.}
Physically, stronger turbulence (larger $\alpha$) or higher $\dot{M}$ implies a lighter, thinner disk.

Once $\Sigma$ is known, the remaining quantities follow directly:

\[
\begin{aligned}
H &\sim \frac{P}{\Sigma\Omega_K^2}
    \;\propto\;
    \frac{\dot{M}f^4}{\Omega_K},
\\[6pt]
\rho &\sim \frac{\Sigma}{H}
    \;\propto\;
    \alpha^{-1}\,\kappa_{\rm es}^{-2}\,
    \dot{M}^{-2}\,f^{-8}\,\Omega_K,
\\[6pt]
T_c &\sim
\left(\frac{P}{a/3}\right)^{1/4}
    \;\propto\;
    \alpha^{-1/4}\,\kappa_{\rm es}^{-1/4}\,
    \Omega_K^{1/8}\,R^{-3/8},
\\[6pt]
\tau &= \frac{1}{2}\kappa_{\rm es}\Sigma
    \;\propto\;
    \alpha^{-1}\,\kappa_{\rm es}^{-1}\,\dot{M}^{-1}\,
    \Omega_K^{-1}\,f^{-4},
\\[6pt]
\nu &\propto \alpha\,\kappa_{\rm es}^2\,\dot{M}^2\,\Omega_K\,f^8,
\\[6pt]
v_R &= -\,\frac{\dot{M}}{2\pi R\Sigma}
     \;\propto\;
     -\,\alpha\,\kappa_{\rm es}^2\,\dot{M}^2\,\Omega_K\,R^{-1}\,f^4.
\end{aligned}
\]
These relations reproduce the standard \textbf{Region A} results from \citet{1973A&A....24..337S} showing explicitly that $T_c$ is \emph{independent of $\dot{M}$} and only weakly dependent on $M$ and $R$. One very important implication of this derivation is the linear dependence of $H$ on $\dot{M}$. If $\dot{M}$ becomes sufficiently large, $H/R$ ceases to be $\ll 1$, violating the thin-disk assumption.  This leads directly to the \textbf{Eddington limit breakdown}. The radiation pressure dominance of Region A implies that a significant fraction of the local vertical force balance is supported by radiation. The radiative flux at each radius is
\[
F = \sigma_{\rm sb} T_{\rm eff}^4 = \frac{3GM\dot{M}}{8\pi R^3}
\left[1 - \left(\frac{R_{\rm in}}{R}\right)^{1/2}\right].
\]
The vertical component of radiation pressure exerts an upward acceleration on the gas:
\[
a_{\rm rad} = \frac{\kappa_{\rm es} F}{c}.
\]
For the disk to remain gravitationally bound, this must be smaller than the vertical component of gravity at $z = H$:
\[
a_{\rm grav} = \Omega_K^2 H.
\]
Equating these defines the condition for the \textbf{local Eddington limit}:
\begin{equation}
\frac{\kappa_{\rm es} F}{c} \;=\; \Omega_K^2 H.
\end{equation}
Using $F$ from above gives
\[
H \;=\; \frac{3}{8\pi}\frac{\kappa_{\rm es}}{c}\,\frac{GM\dot{M}}{R^3}\,\frac{1}{\Omega_K^2}
\;\simeq\;
\frac{3}{8\pi}\frac{\kappa_{\rm es}\dot{M}}{c}.
\]
Notably, $H$ becomes independent of radius --- the disk inflates to a constant scale height once radiation pressure dominates completely.  
When $H/R \sim 1$, the thin-disk approximation breaks down, and vertical radiative forces can drive powerful outflows.

This critical point roughly corresponds to an accretion rate near the \textbf{Eddington accretion rate}:
\[
\dot{M}_{\rm Edd} = \frac{4\pi GM}{\eta\,\kappa_{\rm es}\,c},
\]
where $\eta$ is the radiative efficiency ($\sim 0.1$ for thin disks).  
When $\dot{M} \gtrsim \dot{M}_{\rm Edd}$, the disk becomes geometrically thick and radiatively inefficient, transitioning to a \textbf{slim disk} or \textbf{super-Eddington} accretion flow.


\begin{bigidea}
\textbf{Summary of Region A: Radiation Pressure + Electron Scattering}
This region corresponds to the inner, radiation–dominated part of the disk near compact accretors.
Its breakdown near the Eddington limit marks the transition to
\textbf{slim disks} and the onset of thermal–viscous instabilities.
\end{bigidea}

The final solution for this region corresponds to the following
\begin{equation}
\begin{aligned}
\Sigma &= 48\;
\alpha_{-1}^{-1}\,
...
\quad {\rm cm\,s^{-1}}, \\[6pt]
\text{with}\quad
f &= 1 - \sqrt{\frac{R_{\rm in}}{R}},
\end{aligned}
\end{equation}
where $\alpha_{-1} = \alpha / 0.1$.

\subsection{Region B: Gas Pressure \& Electron Scattering}
\label{sec:region_b}

We now turn to the next regime of the $\alpha$–disk model, where the disk remains \textbf{electron–scattering dominated} in opacity but \textbf{gas pressure} provides the dominant support against gravity. This typically applies to intermediate radii in luminous accretion disks, beyond the radiation–pressure–dominated zone but before the outer free–free opacity region takes over.
...
\end{equation}
As before, angular–momentum conservation gives
\begin{equation}
\nu \Sigma = \frac{\dot{M}}{3\pi}
\left[1 - \left(\frac{R_{\rm in}}{R}\right)^{1/2}\right]
\;\equiv\; \dot{M}\, f^4,
\end{equation}
and the $\alpha$–prescription implies
\[
\nu = \alpha c_s H = \alpha \frac{c_s^2}{\Omega_K}.
\]
...
\[
H = \frac{c_s}{\Omega_K}
= \frac{1}{\Omega_K}\left(\frac{k_B T_c}{\mu m_p}\right)^{1/2}.
\]
For an optically thick, scattering–dominated disk, the diffusive flux gives
\[
T_c^4 = \frac{3}{8}\,\kappa_{\rm es}\,\Sigma\,T_{\rm eff}^4,
\qquad
\sigma_{\rm sb} T_{\rm eff}^4 = \frac{3GM\dot{M}}{8\pi R^3}f^4.
\]
Combining these yields
\[
T_c^4 \propto \kappa_{\rm es}\,\Sigma\,\frac{GM\dot{M}}{R^3}\,f^4.
\]
Using $P = \rho k_B T_c / \mu m_p$ and $\rho \sim \Sigma / 2H$,
\[
P \sim \frac{\Sigma k_B T_c}{2H \mu m_p}.
\]
Since $\nu = \alpha c_s H = \alpha (k_B T_c / \mu m_p) H / \Omega_K$, we can combine
\[
\nu \Sigma \sim \dot{M} f^4
\quad \Rightarrow \quad
\alpha\,\frac{k_B T_c}{\mu m_p}\,\frac{H \Sigma}{\Omega_K} \sim \dot{M}f^4.
\]
Replacing $H = c_s / \Omega_K \propto T_c^{1/2}\Omega_K^{-1}$ and eliminating $\Sigma$ using the diffusion relation gives the closure conditions necessary to solve for all scalings. After substitution and simplification, one finds the characteristic dependences
\[
\boxed{
\begin{aligned}
T_c &\propto \alpha^{-1/5}\,\dot{M}^{2/5}\,M^{3/10}\,R^{-9/10}\,f^{2/5},\\[3pt]
\Sigma &\propto \alpha^{-4/5}\,\dot{M}^{3/5}\,M^{1/5}\,R^{-3/5}\,f^{3/5},\\[3pt]
H &\propto \alpha^{-1/10}\,\dot{M}^{1/5}\,M^{9/10}\,R^{21/20}\,f^{1/5},\\[3pt]
\rho &\propto \alpha^{-7/10}\,\dot{M}^{2/5}\,M^{-7/10}\,R^{-33/20}\,f^{1/5},\\[3pt]
\tau &\propto \alpha^{-4/5}\,\dot{M}^{3/5}\,M^{1/5}\,R^{-3/5}\,f^{3/5}.
\end{aligned}
}
\]
The disk remains geometrically thin and gas–pressure supported, with moderate optical depth.
Using the same cgs normalization as \citet{FrankKingRaine2002}, we obtain
\begin{equation}
\boxed{
\begin{aligned}
\Sigma &= 5.2\times10^{3}\;
...
\end{equation}
where $\alpha_{-1}=\alpha/0.1$, $m_1=M/M_\odot$, $r_6=R/10^6\,{\rm cm}$,and $\dot{M}_{17}=\dot{M}/10^{17}\,{\rm g\,s^{-1}}$.

In this regime, the disk’s vertical support comes from gas pressure,
but the opacity is still dominated by Thomson scattering.
Unlike Region~A, the midplane temperature $T_c$ now \emph{increases with}~$\dot{M}$, and
the surface density also rises with accretion rate.
The disk remains geometrically thin ($H/R\ll1$) and stable under modest perturbations,
making Region~B the archetypal case for many X–ray binary and AGN disks.

\begin{bigidea}
\textbf{Summary of Region B: Gas Pressure + Electron Scattering}
\[
\Sigma \propto \alpha^{-4/5}\,\dot{M}^{3/5}\,M^{1/5}\,R^{-3/5}\,f^{3/5},
\quad
T_c \propto \alpha^{-1/5}\,\dot{M}^{2/5}\,M^{3/10}\,R^{-9/10},
\]
representing a gas–pressure, constant–opacity disk that smoothly connects the inner
radiation–pressure zone to the outer free–free (Kramers) regime.
\end{bigidea}
...
\]
With $\nu=\alpha c_s^2/\Omega_K$ and $c_s^2=k_B T_c/(\mu m_p)$, the steady condition $\nu\Sigma = (\dot M/3\pi)\,f$ (free inner torque, cf.\ eq.~\eqref{eq:thin_disk_viscous}) and radiative diffusion $T_c^4 = (3/8)\kappa\Sigma T_{\rm eff}^4$ with $\sigma_{\rm sb}T_{\rm eff}^4 = (3GM\dot{M}/8\pi R^3)\,f$ close the system. Solving these relations gives the classic Region~C dependences (absorbing order–unity constants into the normalizations):
\[
\boxed{
\begin{aligned}
\Sigma &\;\propto\; 
\alpha^{-4/5}\,\kappa_0^{-1/10}\,
\dot{M}^{7/10}\,
M^{1/4}\,R^{-3/4}\,
f^{7/10},
\\[4pt]
T_c &\;\propto\;
\alpha^{-1/5}\,\kappa_0^{1/10}\,
\dot{M}^{3/10}\,
M^{1/4}\,R^{-3/4}\,
f^{3/10},
\\[4pt]
H &\;=\;\frac{c_s}{\Omega_K}\;\propto\;
\alpha^{-1/10}\,\kappa_0^{1/20}\,
\dot{M}^{3/20}\,
M^{-3/8}\,R^{9/8}\,
f^{3/20},
\\[4pt]
\rho &\;=\;\frac{\Sigma}{2H}\;\propto\;
\alpha^{-7/10}\,\kappa_0^{-3/20}\,
\dot{M}^{11/20}\,
M^{5/8}\,R^{-15/8}\,
f^{2/5},
\\[4pt]
\tau &\;=\;\tfrac{1}{2}\kappa\Sigma\;\propto\;
\alpha^{-4/5}\,\kappa_0^{11/20}\,
\dot{M}^{1/5}\,
f^{1/20}
\quad(\text{notably independent of }M,R),
\\[4pt]
\nu &\;=\;\alpha\frac{c_s^2}{\Omega_K}\;\propto\;
\alpha^{4/5}\,\kappa_0^{1/10}\,
\dot{M}^{3/10}\,
M^{-1/4}\,R^{3/4}\,
f^{3/10},
\\[4pt]
v_R &\;=\;-\frac{\dot{M}}{2\pi R\Sigma}\;\propto\;
\alpha^{4/5}\,\kappa_0^{1/10}\,
\dot{M}^{3/10}\,
M^{-1/4}\,R^{-1/4}\,
f^{-7/10}.
...
Introducing the usual normalizations
\[
\alpha_{-1}=\frac{\alpha}{0.1},\quad
\dot{M}_{17}=\frac{\dot M}{10^{17}\,{\rm g\,s^{-1}}},\quad
m_1=\frac{M}{M_\odot},\quad
r_6=\frac{R}{10^6\,{\rm cm}},\quad
\kappa_{24}=\frac{\kappa_0}{5\times10^{24}},
\]
the Region~C solution may be written in the compact scaling form
\begin{equation}
\boxed{
\begin{aligned}
\Sigma &\;=\; \Sigma_0\;
...
m_1^{-1/4}\,
r_6^{-1/4}\,
f^{-7/10}\quad [{\rm cm\,s^{-1}}],
\\[6pt]
&\text{with}\;\; f \;\equiv\; 1-\sqrt{\frac{R_{\rm in}}{R}}.
\end{aligned}
}
\end{equation}
Here the coefficients $(\Sigma_0, T_{c,0}, H_0, \rho_0, \tau_0, \nu_0, v_{R,0})$ are order–unity numbers that depend only on the adopted constants ($\mu$, exact $\kappa_0$, geometric factors); they can be fixed to your preferred convention (e.g.\ \citealt{FrankKingRaine2002}) by inserting $\mu\simeq0.62$ and $\kappa_0=5\times10^{24}$ and carrying the cgs algebra.
...
\end{enumerate}
Each of these boundaries defines a locus in the $\dot{M}$–$R$ plane, as illustrated in Figure~\ref{fig:accretion_regimes}. These curves determine where the physical assumptions underlying Regions~A–C change.

\subsubsection{The Radiation Pressure Transition}

Radiation pressure dominates when $P_{\rm rad } \gg P_{\rm gas}$. We know that
\[
P_{\rm rad} = \frac{a}{3}T_c^4
\quad\text{and}\quad
P_{\rm gas} = \frac{\rho k_B T_c}{\mu m_p}.
\]
Setting $P_{\rm rad}=P_{\rm gas}$ gives the critical density:
\[
\boxed{
\rho_{\rm crit} = \frac{a \mu m_p}{3 k_B} T_c^3.
}
\]
\textbf{Using the gas–pressure, electron–scattering scalings from Region~B,}
\[
T_c \propto \alpha^{-1/5}\dot{M}^{2/5}M^{3/10}R^{-9/10}f^{2/5}, \quad
\rho \propto \alpha^{-7/10}\dot{M}^{2/5}M^{-7/10}R^{-33/20}f^{1/5},
\]
we find the transition line by solving for $\dot M$ such that $\rho=\rho_{\rm crit}$. After algebraic elimination of $T_c$, the locus of the pressure transition is:
\[
\boxed{
\dot{M}_{P_{\rm rad}=P_{\rm gas}} \;\propto\;
\alpha^{-1}\,
M^{-1/8}\,
R^{21/16}\,f^{-9/8}.
}
\]
Numerically, adopting fiducial constants ($\mu=0.62$, $\kappa_{\rm es}=0.34$) and normalizing to cgs units, this corresponds roughly to
\[
\dot{M}_{P_{\rm rad}=P_{\rm gas}}
\simeq 6\times10^{16}\;
\alpha_{-1}^{-1}\;
m_1^{-1/8}\;
r_6^{21/16}\;
f^{-9/8}\quad {\rm g\,s^{-1}}.
\]
To the left of this curve (smaller $R$ or larger $\dot{M}$),\textbf{ radiation pressure dominates ($P_{\rm rad}>P_{\rm gas}$, Region~A);}
to the right,\textbf{ gas pressure dominates (Regions~B and~C).}

\subsubsection{The Opacity Transition}
The second boundary marks where the opacity changes from electron scattering to free–free absorption:
\[
\kappa_{\rm es} = \kappa_{\rm ff} = \kappa_0 \rho T_c^{-7/2}.
\]
Solving for the transition condition gives
\[
\rho_{\rm crit} = \frac{\kappa_{\rm es}}{\kappa_0} T_c^{7/2}.
\]
Using again the gas–pressure, electron–scattering scalings from Region~B for $\rho$ and $T_c$, we find:
\[
\alpha^{-7/10}\dot{M}^{2/5}M^{-7/10}R^{-33/20}f^{1/5}
\;\propto\;
T_c^{7/2}
\;\propto\;
\alpha^{-7/10}\dot{M}^{7/5}M^{21/20}R^{-63/20}f^{7/5}.
\]
Simplifying, the condition $\kappa_{\rm es}=\kappa_{\rm ff}$ yields the approximate scaling
\[
\boxed{
\dot{M}_{\kappa_{\rm es}=\kappa_{\rm ff}}
\;\propto\;
M^{-1/4}\,
R^{15/8}\,f^{-3/4}.
}
\]
Normalizing with fiducial constants ($\kappa_{\rm es}=0.34$, $\kappa_0=5\times10^{24}$), we find numerically:
\[
\dot{M}_{\kappa_{\rm es}=\kappa_{\rm ff}}
\simeq 3\times10^{16}\;
m_1^{-1/4}\;
...
f^{-3/4}\quad{\rm g\,s^{-1}}.
\]
\textbf{Inside this line (smaller radii or higher $\dot{M}$), electron scattering dominates (Regions~A and~B);
beyond it, Kramers opacity dominates (Region~C).}

\subsubsection{The Temperature Transition}

Finally, the disk ceases to be fully ionized when $T_c \simeq 10^4\,{\rm K}$, marking the outermost boundary of the thin, ionized $\alpha$–disk. Using the Region~C scaling $T_c \propto \alpha^{-1/5}\dot{M}^{3/10}M^{1/4}R^{-3/4}$, we can invert for the locus:
\[
\boxed{
\dot{M}_{T_c=10^4{\rm K}} \;\propto\;
\alpha^{2/3}\,
M^{-5/4}\,
R^{5/2}.
}
\]
At radii larger than this, the disk becomes partially neutral and new opacity sources (molecular and dust) dominate. 

Combining these three transition loci produces the standard regime diagram
shown in Figure~\ref{fig:accretion_regimes}.
Each region corresponds to one of the analytic $\alpha$–disk solutions:

\[
\begin{array}{ll}
\textbf{Region A:} & P_{\rm rad},~\kappa_{\rm es} \quad (\text{inner, high } \dot{M})\\[3pt]
\textbf{Region B:} & P_{\rm gas},~\kappa_{\rm es} \quad (\text{intermediate})\\[3pt]
\textbf{Region C:} & P_{\rm gas},~\kappa_{\rm ff} \quad (\text{outer, cool})
\end{array}
\]

These analytic boundaries depend only weakly on $\alpha$ and $M$, and approximately follow the power-law relations:
\[
\dot{M} \propto R^{1.3}\ \text{for the pressure boundary,}
\qquad
\dot{M} \propto R^{1.9}\ \text{for the opacity boundary.}
\]
Their intersections set the typical radii of transitions indicated in Figure~\ref{fig:accretion_regimes}.
The vertical lines correspond roughly to characteristic stellar scales:
the innermost stable orbit of a black hole ($R\!\sim\!10^6{\rm cm}$),
the radius of a white dwarf ($\sim\!10^9{\rm cm}$),
and typical Roche lobe sizes for cataclysmic variables and X–ray binaries.
